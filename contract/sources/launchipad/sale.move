...
module swift_nft::sale {
    ...
    public fun create_empty_sale<Item: key+store, Market: store>(
        market: Market,
        ctx: &mut TxContext
    ): Sale<Item, Market> {
        ...
        sale_event::sale_create_event<Item, Market>(sale_id, false);
        return new_sale
    }

    public fun create_sale<Item: key+store, Market: store>(
        nfts: vector<Item>,
        whitelist: bool,
        market: Market,
        ctx: &mut TxContext
        ...
        let sale_id = object::id(&new_sale);
        sale_event::sale_create_event<Item, Market>(sale_id, whitelist);
    ...
    public fun list_item<Item: key+store, Market: store>(sales: &mut Sale<Item, Market>, nft: Item) {
        ...
        sale_event::item_list_event<Item, Market>(object::id(sales), vector::singleton(nft_id))
    }
    ...
    public fun unlist_item<Item: key+store, Market: store>(
        sales: &mut Sale<Item, Market>,
        nfts: vector<ID>
    ): vector<Item> {
        let sale_id = object::id(sales);
        let length = vector::length(&nfts);
    ...
    public fun whitelist_on<Item: key+store, Market: store>(sales: &mut Sale<Item, Market>) {
        ...
        sale_event::wl_status_change_event<Item, Market>(object::id(sales), true)
    }

    public fun whitelist_off<Item: key+store, Market: store>(sales: &mut Sale<Item, Market>) {
        sales.whitelist = false;
        sale_event::wl_status_change_event<Item, Market>(object::id(sales), false)
    }
