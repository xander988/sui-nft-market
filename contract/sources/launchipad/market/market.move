module swift_nft::slingshot_market {
    ...
    public entry fun create<Item: key+store,CoinType>(
        ...
        create_muti_sales_market<Item,CoinType>(collection,admin,live,whitelists,array,start_times,end_times,prices,ctx)
    }

    public  fun create_muti_sales_market<Item: key+store,CoinType>(collection:ID, admin: address,live: bool,whitelists:vector<bool>,nft_vec: vector<vector<Item>>,start_times:vector<u64>,end_times: vector<u64>,prices: vector<u64>,ctx: &mut TxContext){
        ...
        while ( i < length ){
            ...
            slingshot_market_event::market_create_event<Item,SlingshotMarket<Item,CoinType>>(object::id(&market), start_time,
                end_time,
                price,tx_context::sender(ctx));
    ...
    public entry  fun create_market<Item: key+store,CoinType>(collection:ID, admin: address,live: bool,nfts: vector<Item> ,whitelist:bool,start_time:u64,end_time: u64,price:u64,ctx: &mut TxContext){
        let market=SlingshotMarket<Item,CoinType>{
            id: object::new(ctx),
            start_time,
            end_time,
    ...
    public fun remove_sale<Item: key+store,CoinType>(sling: &mut Slingshot<Item,SlingshotMarket<Item,CoinType>>,sale_id: ID,ctx: &mut TxContext){
        ...
        slingshot_market_event::remove_sale_event<Item,SlingshotMarket<Item,CoinType>>(object::id(sling),sale_id,tx_context::sender(ctx));
        transfer::transfer(sale,tx_context::sender(ctx));
    }
    public entry  fun adjust_price<Item: key+store,CoinType>(slingshot: &mut Slingshot<Item,SlingshotMarket<Item,CoinType>>,sale_id: ID,price: u64,ctx: &mut TxContext){
        ...
        slingshot_market_event::adjust_price_event<Item,SlingshotMarket<Item,CoinType>>(object::id(slingshot),sale_id,price,tx_context::sender(ctx))
    }

    public fun collect<Item: key+store,CoinType>(slingshot: &mut Slingshot<Item,SlingshotMarket<Item,CoinType>>,sale_id: ID, receiver: address,ctx :&mut TxContext ){
        let borrow_mut_sale=slingshot::borrow_mut_sales(slingshot,sale_id,ctx);
    ...
    public entry fun purchase<Item: key+store,CoinType>(slingshot: &mut Slingshot<Item,SlingshotMarket<Item,CoinType>>,sale_id: ID,item_id: ID,buyer_funds:&mut  Coin<CoinType>,ctx: &mut TxContext){
        let borrow_sale=slingshot::borrow_sales(slingshot,sale_id,ctx);
        ...
        pay::join(funds,fund);
        slingshot_market_event::purchase_event<Item,SlingshotMarket<Item,CoinType>>(object::id(slingshot),sale_id,item_id,price,tx_context::sender(ctx));
        transfer::transfer(item,tx_context::sender(ctx));
    }

    public entry fun  purchase_via_whitelist<Item: key+store,CoinType>(slingshot: &mut Slingshot<Item,SlingshotMarket<Item,CoinType>>,sale_id: ID,wl: WhiteListToken<Item,SlingshotMarket<Item,CoinType>>,item_id: ID,buyer_funds:&mut  Coin<CoinType>,ctx: &mut TxContext){
