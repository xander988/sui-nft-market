...
module swift_nft::slingshot {
    ...
    public fun create_slingshot<Item: key+store, Market: store>(
        collection_id: ID,
        admin: address,
        live: bool,
        sales: vector<Sale<Item, Market>>,
        ctx: &mut TxContext
    ) {
        ...
        slingshot_event::slingshot_create_event<Item, Market>(slingshot_id, sale_ids);

        vector::destroy_empty(sales);
        transfer::share_object(slingshot);
    }

    public fun add_muti_sales<Item: key+store, Market: store>(
        slingshot: &mut Slingshot<Item, Market>,
        sales: vector<Sale<Item, Market>>,
        ctx: &mut TxContext
    ) {
        ...
        slingshot_event::sales_add_event<Item, Market>(object::id(slingshot), sale_vec);
        vector::destroy_empty(sales);
    }

    public fun remove_muti_sales<Item: key+store, Market: store>(
        slingshot: &mut Slingshot<Item, Market>,
        sales: vector<ID>,
        ctx: &mut TxContext
    ): vector<Sale<Item, Market>> {
        assert!(slingshot.admin == tx_context::sender(ctx), EOperateNotAuth);
        let length = vector::length(&sales);
        ...
        slingshot_event::sales_remove_event<Item, Market>(object::id(slingshot), sales);
        result_sale
    }


    public(friend) fun remove_sales<Item: key+store, Market: store>(
        slingshot: &mut Slingshot<Item, Market>,
        sale_id: ID
    ): Sale<Item, Market> {
        slingshot_event::sales_remove_event<Item, Market>(object::id(slingshot), vector::singleton(sale_id));
        object_table::remove(&mut slingshot.sales, sale_id)
    }

    public fun borrow_mut_sales<Item: key+store, Market: store>(
        slingshot: &mut Slingshot<Item, Market>,
        sales_id: ID,
        ctx: &mut TxContext
    ): &mut Sale<Item, Market> {
        assert!(slingshot.admin == tx_context::sender(ctx), EOperateNotAuth);
        slingshot_event::sales_borrow_event<Item, Market>(object::id(slingshot), tx_context::sender(ctx));

        object_table::borrow_mut(&mut slingshot.sales, sales_id)
    }

    public fun borrow_sales<Item: key+store, Market: store>(
        slingshot: &Slingshot<Item, Market>,
        sales_id: ID,
        ctx: &mut TxContext
    ): &Sale<Item, Market> {
        assert!(slingshot.admin == tx_context::sender(ctx), EOperateNotAuth);
    ...
    public fun update_admin<Item: key+store, Market: store>(
        slingshot: &mut Slingshot<Item, Market>,
        admin: address,
        ctx: &mut TxContext
    ) {
        assert!(slingshot.admin == tx_context::sender(ctx), EOperateNotAuth);
        slingshot_event::admin_update_event<Item, Market>(object::id(slingshot), admin);
        slingshot.admin = admin
    }

    public fun modity_status<Item: key+store, Market: store>(
        slingshot: &mut Slingshot<Item, Market>,
        status: bool,
        ctx: &mut TxContext
    ) {
        assert!(slingshot.admin == tx_context::sender(ctx), EOperateNotAuth);
        slingshot_event::live_change_event<Item, Market>(object::id(slingshot), status);
